...
def __aplicar_correlacao_parcial(altura_imagem : int, largura_imagem : int, inicio : int, fim : int, mascara):
    shm_entrada, imagem_entrada = get_memoria_compartilhada(MemoriaCompartilhada.IMAGEM_ENTRADA, altura_imagem, largura_imagem)
    shm_saida, imagem_saida = get_memoria_compartilhada(MemoriaCompartilhada.IMAGEM_SAIDA, altura_imagem, largura_imagem)
...
def aplicar_correlacao(shm_entrada, imagem_entrada : np.ndarray, mascara : np.ndarray, num_processo : int):
    altura_imagem, largura_imagem = imagem_entrada.shape
    
    shm_saida, imagem_saida = criar_imagem_saida(altura_imagem, largura_imagem)

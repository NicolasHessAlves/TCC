...
int main(int argumentos_contagem, char *argumentos_valores[])
{
    ...
    if (temp != "Threads:" || threads <= 0) {
        cerr << "Erro: Número de threads inválido. Deve ser maior que 0.\n";
        return 1;
    }
    ...
    for (const string &conteudo_linha : linhas)
    {
        while (execucao_atual < quantidade_execucoes)
        {
            ...
            case 'L':
            {
                ...
                imagem_saida = aplicar_limiarizacao(imagem, (uchar)valor_limiar, threads);
                break;
            }
            case 'H':
            {
                imagem_saida = aplicar_equalizacao_histograma(imagem, threads);
                break;
            }
            case 'C':
            {
                ...
                imagem_saida = aplicar_correlacao(imagem, mascara, threads);
                break;
            }
            }
